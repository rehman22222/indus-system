...
export function useManagementStats(date: string) {
  ...
  const fetchStats = useCallback(async () => {
    try {
      ...
      // 'arrived' = checked in and in the waiting queue; 'waiting' = booked but
      // not yet arrived. Kept mutually exclusive so the two tiles don't double
      // count a single appointment.
      const arrived = appointments?.filter(a => a.status === 'waiting').length || 0;
      const waiting = appointments?.filter(a => a.status === 'confirmed').length || 0;
  ...
  useEffect(() => {
    ...
    // Real-time subscription — wakes us up on Supabase writes.
  ...
  const fetchDoctors = useCallback(async () => {
    try {
      ...
      const doctorsWithStats: DoctorWithStats[] = (doctorsData || []).map((doc: any) => {
        ...
        // Slots consumed = appointments that still occupy capacity. Cancelled
        // and no-show appointments free their slot back up, so exclude them.
        const activeAppointments = docAppointments.filter(
        ...
        return {
          ...
          seen: completed,
          remaining: Math.max(0, totalQuota - activeAppointments.length),
          physicalCount: physical,
          videoCount: video,
