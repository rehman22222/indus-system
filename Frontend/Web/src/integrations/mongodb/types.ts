...
export interface Doctor {
    ...
    // Optional embedded department (only present when a join is selected)
...
export interface Appointment {
    ...
    // Optional embedded resources — only present when the query selects
    // `patient:patients(...)` / `doctor:doctors(...)`. Both `name` (alias
    // of full_name) and `full_name` are kept optional for compatibility.
    patient?: {
        id: string;
        patient_id?: string;
        name?: string;
        ...
        dob?: string;
        sex?: Sex;
        // Not real columns (no age/gender on patients — derived from
        // dob/sex). Kept optional so legacy views compile; undefined at
        // runtime and their UI blocks simply don't render.
        age?: number;
        gender?: string;
        blood_group?: BloodGroup;
        allergies?: string[];
    ...
    // Set by a check-in step in some deployments; not selected here, so
    // undefined at runtime (the Vitals tab shows its empty state).
