...
async function callGroq(
    ...
    if (!apiKey) {
        console.warn('VITE_GROQ_API_KEY not set — Groq AI features disabled.');
        return '';
    }
    ...
    try {
        ...
        if (!response.ok) {
            ...
            console.warn('Groq API error:', err?.error?.message || response.statusText);
            return '';
        }
        ...
        console.warn('Groq API unavailable:', error);
        return '';
    }
}
