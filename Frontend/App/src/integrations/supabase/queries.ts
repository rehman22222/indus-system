/**
 * Scoped Supabase query selects
 * Never use .select('*') - always specify columns needed
 */

// Doctor queries
export const DOCTOR_LIST_SELECT = 'id, full_name, specialty, department_id, daily_physical_quota, daily_video_quota, profile_image_url, consultation_fee, is_active';
...
// Appointment queries
export const APPOINTMENT_LIST_SELECT = 'id, token, appointment_date, appointment_time, appointment_type, status, chief_complaint, no_show_score, patient:patients(id, full_name, phone), doctor:doctors(id, full_name, specialty)';
export const APPOINTMENT_DETAIL_SELECT = 'id, token, appointment_date, appointment_time, appointment_type, status, chief_complaint, notes, diagnosis, check_in_time, consultation_start_time, consultation_end_time, no_show_score, governance_status, governance_notes, video_room_url, patient:patients(id, indus_id, full_name, phone, dob, sex, blood_group, allergies, current_medications), doctor:doctors(id, full_name, specialty, phone)';
